...
const flexBySpan: Record<Project["span"], string> = {
  wide: "flex-[1_1_calc(50%-11px)] max-[700px]:flex-[1_1_100%]",
  narrow: "flex-[1_1_calc(50%-11px)] max-[700px]:flex-[1_1_100%]",
  full: "flex-[1_1_100%]",
};
...
export function ProjectCard({
  ...
  const header = (
    <div className="flex flex-wrap items-baseline gap-x-3 gap-y-2">
      <h3 className="m-0 text-[23px] font-semibold tracking-[-0.025em] max-[700px]:text-[20px]">
        {project.title}
  ...
  const description = expanded ? (
    ...
    <p
      ...
      className="clamp-desc text-pretty m-0 text-[15.5px] leading-[1.65] text-text-muted max-[700px]:text-[14.5px]"
    >
      {project.description}
    </p>
  );
  ...
  const writeUp = expanded ? (
    <div className="flex flex-col gap-[22px]">
      ...
      {(project.whatBroke || project.whatIdDoDifferently) && (
        <div className="flex max-w-[min(74ch,100%)] flex-col gap-3 rounded-[12px] border border-[rgba(242,237,228,0.09)] bg-[rgba(194,96,58,0.05)] px-[18px] py-4">
          {project.whatBroke && (
            <div>
  ...
  if (expanded) {
    return (
      <article className="flex flex-col">
        {media}
        <div className="flex flex-col gap-[22px] p-8 max-[700px]:p-5">
          {header}
          {description}
          {writeUp}
          {footer}
